...
k_esp = np.zeros(time, dtype=float)
e_esp = np.zeros(time, dtype=float)
j = 0
for i in range(time):
    # Variables
    ...
    k_esp[i] = kineticEnergy(onda)
    e_esp[i] = totalEnergy(onda, pot[int(2.5*N/5)])
...
# Representacion del valor esperado de Energia Cinetica y Energia Total
plt.plot(np.arange(start=0, stop=time, step=1, dtype=int), k_esp)
plt.plot(np.arange(start=0, stop=time, step=1, dtype=int), e_esp)
plt.xlabel("Tiempo (s)")
plt.ylabel("Energía Cinética")
plt.legend(["Energía Cinética", "Energía Total"], loc='lower right')
plt.title("Energía cinética en cada instante")
plt.show()
